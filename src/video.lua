...
video_renderer.load = function(filename)
    ...
    local temp = ffi.new("uint64_t[1]")
    local mpv_rd_ptr = ffi.new("struct mpv_render_context *[1]", { ffi.cast("struct mpv_render_context *", temp) })
    if (mpv.mpv_render_context_create(mpv_rd_ptr, mpv_handle, params) < 0) then 
        pprint("[Error] mpv failed to initialize mpv render context")
        return
    end
