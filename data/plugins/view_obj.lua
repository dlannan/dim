...
local function OBJ_on_hide(doc)
    if (doc.model) then
...
local function OBJ_on_close(doc)
    if (doc.model) then
        local model = doc.model
...
local function OBJ_load(self, filename)
    self.on_hide = OBJ_on_hide
    self.on_close = OBJ_on_close
    ...
    local params = {
        do_load = function(model_load)
            ...
            local OBJloader              = require("lua.loaders.objloader.objloader")
            return OBJloader.load_obj(model_load.filename, model_load.params)
        end,
        type = "obj",
    }
    params.on_load = function(model, params)
        local cam_count = utils.tcount(cammgr.cameras())
        local cam_name = "obj_cam_" .. model.name .. "_" .. (cam_count or 0)
...
tinsert(Doc.loaders, { loader = OBJ_load, exts = threed_models.files })


local function OBJ_on_mouse_moved(self, x, y, dx, dy)
    self.hovered = true
end

local function OBJ_draw(self)
    ...
    if (self.doc.model and self.doc.model.loaded == true) then
        if (self.view and self.mouse_mapped == nil) then
            self.view.on_mouse_moved = OBJ_on_mouse_moved
...
local function OBJ_on_mouse_wheel(self, y)
...
tinsert(DocView.drawers, OBJ_draw)
tinsert(DocView.on_mouse_wheels, OBJ_on_mouse_wheel)

core.register_extension("obj")
