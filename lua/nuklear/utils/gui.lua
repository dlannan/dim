...
local nuklear 	= require("nuklear.style")
...
nuklear_gui.setup_gui = function( self, gui_quad, camera_url, scale_texture )
	...
	local newwidth, newheight = renderer.get_size()
...
nuklear_gui.widget_panel_simple = function (self, title, left, top, width, height, panel_function, ctx)

    local flags = 0
	local newx, newy, wide, high
	local winshow = nuklear.begin_window( title , left, top, width, height, flags)
	if( winshow == true) then 
	    if(panel_function) then panel_function(ctx, left, top, width, height) end
		newx, newy, wide, high = nuklear.get_bounds_window()
		nuklear.end_window()
	end
...
--------------------------------------------------------------------------------
-- NOTE: Because this nuklear all runs within a single window, then these "windows" are groups
...
nuklear_gui.widget_button = function (self, text, left, top, width, height)
	...
	if(nuklear.begin_window( text , x, y, width+10, height+8, self.flags.NK_WINDOW_NO_SCROLLBAR) == true) then
...
nuklear_gui.faicon_tooltip = function( self, ctx, obj, align, font_override) 
    ...
    if(obj.hoverctr > 100) then 
		...
		if (nk.nk_input_is_mouse_hovering_rect(ctx.input, bounds)) then
			nk_tooltip(ctx, obj.name, align or 0)
		end	
    end 
	wdgts:set_font( font_override or style.fa_font )
...
--------------------------------------------------------------------------------
local COL_RGB, COL_HSV = 0, 1
local col_mode = COL_RGB
local space = ffi.new("struct nk_rect[1]") -- (bounds.x, bounds.y-5, bounds.w, 25)

nuklear_gui.picker_color_complex = function(ctx, rgba, bgra)

	local canvas = nk.nk_window_get_canvas(ctx)
	local in_color = nk.nk_rgba_u32(rgba)
	-- This is specifically for handling defold bgr colors
	if(bgra) then 
		in_color.r, in_color.b = in_color.b, in_color.r
	end

	nk.nk_layout_row_dynamic(ctx, 25, 1)
	nk.nk_widget(space, ctx)
	nk.nk_fill_rect(canvas, space[0], 0, in_color)

	nk.nk_layout_row_dynamic(ctx, 120, 1)
    local combo_color2 = nk.nk_color_cf(in_color)
	combo_color2 = nk.nk_color_picker(ctx, combo_color2, nk.NK_RGBA)

    nk.nk_layout_row_dynamic(ctx, 25, 2)
    col_mode = nk.nk_option_label(ctx, "RGB", col_mode == COL_RGB) or COL_HSV
    col_mode = nk.nk_option_label(ctx, "HSV", col_mode == COL_HSV) or COL_RGB
	...
	if(bgra) then 
		combo_color2.r, combo_color2.b = combo_color2.b, combo_color2.r
	end
    return nk.nk_color_u32(nk.nk_rgba_cf(combo_color2))
end
