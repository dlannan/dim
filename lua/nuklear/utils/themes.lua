...
-----------------------------------------------------------------------------------
-- Get current state of nk theme
--    fetches the properties in a function you can call to set and then save!!
local function get_theme(theme_name)
    local theme_str = ""
    theme_str = theme_str.."-- -----------------------------------------------------------------------\n"
    theme_str = theme_str.."-- Auto generated theme file. Do not edit!\n"
    theme_str = theme_str.."-- -----------------------------------------------------------------------\n"
    theme_str = theme_str.."nuklear.set_style(2, 255, 0xffffffff)  -- set custom style\n"

    for k,v in ipairs(theme_index_keys) do        
        theme_str = theme_str.."nuklear.set_style_prop(indexes."..tostring(v)..", "..string.format("0x%0x", nuklear.get_style_prop(indexes[v]))..")\n"
    end
    theme_str = theme_str.."nuklear.set_style_table()\n"

    local func = assert(load(theme_str, theme_str, "t" , {indexes=indexes, nuklear=nuklear}))
    themes[theme_name] = func
    return func
end
...
themes.theme_panel = function ( self, font, left, top, width, height, readonly )
	...
	if( nuklear.begin_window( "Theme Editor", x, y, width, height, flags) == true) then
