...
gltf.componentTypes = {               -- (little endian)
	...
	[5122] = {size = 2, fmt = "<h"}, -- signed short
	[5123] = {size = 2, fmt = "<H"}, -- unsigned short
	[5125] = {size = 4, fmt = "<I"}, -- unsigned int
...
local function bufferUnpack(bin, accessorType, componentType, count, stride)
	...
	for i = 0, count - 1 do
		if accessorType == "SCALAR" then
			table.insert(elements, (struct.unpack(fmt, bin, i*componentSize + 1)))
		elseif accessorType == "VEC2" or accessorType == "VEC3" or accessorType == "VEC4" then
			local vec = {}
			for j = 1, numComponents do
				vec[j] = struct.unpack(fmt, bin, i*stride + (j - 1)*componentSize + 1)
			end
		...
		elseif accessorType == "MAT2" or accessorType == "MAT3" or accessorType == "MAT4" then
			...
			for j = 0, numComponents - 1 do
				table.insert(
					mat,
					(struct.unpack(fmt, bin, i*elemSize + j*componentSize + 1))
				)
			end
...
local function getImageData(image)
	...
	error("[Error] Failed to get image uri or data.")
end

-- Adding glb support (mainly buffer loading)
--    Moved to use struct.unpack for generic Luajit and Lua support (Lua 5.3 and above - wont use)
function gltf.new(path)
	local file = nil
	
	local handle, err = io.open(path, "rb")
	if handle == nil then 
		print("[Error] Unable to open file: "..path.. "  with error: "..tostring(err))
		return nil
	end

	-- Detect type in case config/info needs to be set for glb
	if( path:sub(-4, -1) == "gltf") then 
		print("[Info] GLTF File detected.")
		file = handle:read("*a")
		handle:close()
	elseif ( path:sub(-3, -1) == "glb" ) then 
		print("[Info] GLB File detected.")
		file = handle:read("*a")		
		handle:close()        
	end

	if(file == nil) then 
		print("[Error] gltf.new couldnt load file: "..tostring(path))
		return nil
	end

	local asset, bin
	if file:sub(1, 4) == "glTF" then
		assert(struct.unpack("<I", file, 5) == 2, "incompatible glTF verison") -- version
		assert(struct.unpack("<I", file, 9) == file:len()) -- length
		...
		while head < file:len() do
			local chunkLength = struct.unpack("<I", file:sub(head, head + 3))
